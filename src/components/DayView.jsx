...
function getItemsForDay(day) {
  ...
  if (WEIGH_DAYS.has(day)) items.push({
    ...
    label: "Log morning weigh-in", icon: "⚖️", isWeighIn: true,
  });
  items.push(...prefix(NIGHTLY_ALWAYS));
  return items;
}

export { getItemsForDay };

export default function DayView({ checked, toggle, todayCode, showResetConfirm, setShowResetConfirm, resetAll }) {
  ...
  function dayProgress(day) {
    const its = getItemsForDay(day);
    const d   = its.filter(i => checked[i.id]).length;
