        ...
        <div style={{ display:"flex", gap:6, marginBottom:16 }}>
          {[["search","Search / Quick-add"],["manual","Manual entry"]].map(([m,l]) => (
            <button key={m} onClick={() => setMode(m)} style={{ flex:1, padding:"8px",
              ...
              color:mode===m?"#0d0d10":"#777",
              fontFamily:"'DM Sans',sans-serif", fontSize:12 }}>{l}</button>
          ))}
        </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {[
              ...
              <input key={i} type={type} value={val} onChange={e => set(e.target.value)}
                placeholder={ph} style={{ background:"#0d0d10",
                  border:"1px solid rgba(255,255,255,0.08)", borderRadius:10,
                  padding:"10px 14px", color:"#ece9e3", fontSize:13,
...
export default function FoodTab({ foodLog, onUpdateLog, onSubmitDay, foodDatabase, onAddToDatabase }) {
  ...
  const todayData = foodLog[today] || { pre:[], main:[], snack:[], locked:false };
  const isLocked  = !!todayData.locked;
  ...
  // History — all submitted days, most recent first
  ...
  function addFood(section, food) {
    ...
    // Add to personal database if new
